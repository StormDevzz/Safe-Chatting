...
Чат-сервер на C. Всё шифруется (AES-256-GCM).
...
./chat        # подключиться (в другом терминале)
```

## Команды в чате

- `@имя текст` — написать сообщение
- `/users` — кто в сети
- `/exit` — выйти

Сначала `R` (регистрация), потом `L` (логин).

## Структура

```
c/main/       — сервер и клиент
c/connection/ — сеть
c/encryption/ — шифрование
c/chatting/   — логика чата
c/files/      — файлы
```
